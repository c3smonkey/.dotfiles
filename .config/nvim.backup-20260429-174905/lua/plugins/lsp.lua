...
-- Build workspace library for lua_ls
-- We need to give lua_ls the Neovim API definitions
local function get_runtime_path()
  -- Try to get VIMRUNTIME dynamically
  local runtime_paths = vim.api.nvim_list_runtime_paths()
  for _, path in ipairs(runtime_paths) do
    -- Find the one that looks like Neovim's runtime
    if path:match("/share/nvim/runtime$") then
      return path
    end
  end
  -- Fallback to hardcoded Homebrew path
  return "/opt/homebrew/Cellar/neovim/0.12.1/share/nvim/runtime"
end

local runtime_path = get_runtime_path()
local workspace_library = {
  runtime_path,
  runtime_path .. "/lua",
  runtime_path .. "/lua/vim",
  runtime_path .. "/lua/vim/_meta",  -- Critical: Contains vim.* API definitions
