...
keymap.set('n', '<C-Right>', ':vertical resize +2<CR>', opts) -- copy into system clipboard with CTRL + C keymap.set('v', '<C-c>', '"+y', opts) copy into host system clipboard with <leader>y keymap.set('v', '<leader>y', '"*y', opts) prevent x from copying over Vim clipboard keymap.set('n', 'x', '"_x', opts)
